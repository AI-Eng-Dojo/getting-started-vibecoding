...
2. **VS Codeのバージョンを確認する。** メニューの「Code」→「バージョン情報」（Windowsは「ヘルプ」→「バージョン情報」）で確認し、**1.94.0 より古ければ先に更新してください**（「Code」→「更新の確認」）
   ...
   > **なぜ先に確認するのか。** Claude Code拡張には必要なVS Codeのバージョンがあります。**古いままだと、拡張を入れても起動しません。** そしてその症状は、このあと出てくる別の原因（Node.js不足）とよく似ています。**先にここを潰しておくと、うまくいかないときの原因が1つ減ります。**

3. VS Codeの拡張機能から「Claude Code」を検索してインストール
4. **Windowsをお使いの場合のみ**: [nodejs.org](https://nodejs.org/) から**LTS版（推奨版）**をダウンロードしてインストールし、**インストール後にPCを再起動**してください
