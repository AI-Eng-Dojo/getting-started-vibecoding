...
4. 全OS共通で、[nodejs.org](https://nodejs.org/) からLTS版（22以上）をダウンロードしてインストールし、**インストール後にPCを再起動**してください
   ...
   - 後半9のテスト（Playwright）、後半11の `npx wrangler` による公開、後半12のBacklog連携で使います
...
- [ ] **0.1** ClaudeとGitHubのアカウントができている（Gmail連携またはCloudflareアカウント作成を行う場合はGoogleも用意。Cloudflareは当日作成も可。Notion／Backlogは任意）
