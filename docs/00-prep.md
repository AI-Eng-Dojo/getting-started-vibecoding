...
| 項目 | いつ使うか | 必須/任意 | 確認方法 |
|---|---|---|---|
| **VS Code** | 終始 | **必須** | 起動する（→ 0.2） |
| **Claude Code拡張** | 終始 | **必須** | 一言送って応答が返る（→ 0.2） |
| **git** | 後半9で15分ごとに保存 | **必須** | Claude Codeに聞く（→ 0.2の6） |
| **`gh`（GitHub CLI）** | 前半2・3でリポジトリを作成・clone | **必須** | `gh auth status` が通る（→ 0.4） |
| **Node.js** | 後半11のルートB（`npx wrangler`）、後半12のBacklog | **Windowsは必須**／Mac・Linuxは任意 | `node -v` でバージョンが出る（→ 0.2の3） |
| **ブラウザ** | 動作確認・claude.ai・公開URLの確認 | **必須** | ふだん使っているもので可 |

> **Node.jsについて。** WindowsはClaude Code拡張の起動自体に必要なので必須です。Mac・Linuxでは本線（後半11のルートA、後半12のNotion）だけなら不要ですが、**入れておくと後半の選択肢が2つ増えます**（`wrangler`での公開、BacklogとのMCP連携）。ターミナルで `node -v` を実行してバージョンが出なければ、[nodejs.org](https://nodejs.org/) のLTS版を入れておくと当日の幅が広がります。

### アカウント

| 項目 | いつ使うか | 必須/任意 | 備考 |
|---|---|---|---|
| **Claudeアカウント（Pro以上）** | 終始 | **必須** | Max推奨（→ 0.1①） |
| **GitHubアカウント** | 前半2以降ずっと | **必須** | 個人アカウント推奨（→ 0.1②） |
| **Googleアカウント（Gmail）** | 前半2のMCP体験、Cloudflareの認証 | **必須に近い** | 接続しない場合は見学で可（→ 0.1③） |
| **Cloudflareアカウント** | 後半11で公開したものを1時間以上残す場合 | 任意 | Google認証で作成（→ 0.1④） |
| **Notionアカウント** | 後半12でMCP連携を試す場合 | 任意 | 後半12はNotionかBacklogのどちらか片方でよい |
| **Backlogアカウント＋APIキー** | 後半12でNotionではなくBacklogを選ぶ場合 | 任意 | **選ぶなら事前準備が必要**（下記） |

> **後半12でBacklogを選びたい人へ。** 当日この枠は15分しかなく、**アカウント作成とAPIキー発行をその場でやる時間はありません。** Backlogを使いたい人は、前日までに ①Backlogのスペースにログインできること ②「個人設定」→「API」からAPIキーを発行し、手元に控えてあること ③`node -v` が通ること——の3つを済ませておいてください。**どれか1つでも欠ける場合は、当日はNotion側を選んでください。**
...
後半11で、作ったアプリを実際にインターネットへ公開します。**アカウント無しでも公開を試せるルートを用意している**ので、事前準備としては必須ではありません。
...
> **すでにCloudflareアカウントを持っている方は注意してください。** メールアドレスとパスワードで作った既存アカウントがある場合、「Sign in with Google」を使うと**別の空アカウントが新しく作られてしまう**ことがあります。既存アカウントをお使いの方は、無理にGoogle認証へ切り替えず、**いつもの方法でログインしてください。**
...
7. 続けて、**コミットに残る名前とメールアドレス**を確認します。次を送ってください。
...
8. 名前とアドレスを設定します。**メールアドレスはGitHubが用意している「noreplyアドレス」を使います。**
...
| 「上限に達しました」と出た | **前日なら**5時間待てば回復するので問題ありません。**当日出た場合は待てません**——挙手したうえで、`/model` で軽いモデルに落とす／`/clear` で会話を切る／プランを上げる、のどれかをその場で選びます。当日出さないために0.1の注意を守ってください |
...
| コミットで `Author identity unknown` と出た | 0.2の7〜8をやっていない状態です。名前とメールアドレスを設定してください |
