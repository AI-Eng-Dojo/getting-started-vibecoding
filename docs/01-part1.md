...
2. **`vibecoding-0908` という名前で新しいリポジトリを作る** — 今日使うリポジトリは、ホームフォルダの下の `github` フォルダ（`~/github/`。Windowsでは `C:\Users\⟨自分の名前⟩\github\`）にまとめて作ります。無ければClaude Codeが自動で作ります。**普段使っているリポジトリの置き場所がすでにある人は、無理に合わせず、そちらに作って構いません。**
   ...
   **`gh`が使える場合（事前準備4を済ませた人はこちら）**
   ...
   `gh` の認証は事前準備4で済んでいるので、ログインを求められることはないはずです。**求められた場合、またはエラーが出た場合は**、事前準備の「つまずき早見表」の `gh repo create` の行を参照するか、そのままエラーをClaude Codeに貼って「直してください」と頼んでください。
...
ここから休憩を挟んでハンズオン③に入ります。**Skillに何十往復も質問させる、今日いちばん枠を使う場面です。** その先には宿題（自分のアイデアを問い詰めさせる）と、後半2時間45分が控えています。
