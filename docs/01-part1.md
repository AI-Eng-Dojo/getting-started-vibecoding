...
| 0:10–1:15 | **2. ハンズオン① Claude Code基礎** — `vibecoding-0908` を作り、**Vibe Coding → MCP → Agent Skill** の3本柱を一通り触る。簡単なものを1個作る→GmailとMCPでつなぐ→最初のSkill `tsumete` を作ってみる | 動くアプリ1個以上・Gmail下書き1通・`tsumete`(プロジェクトレベル) |
...
| 1:45–2:00 | **休憩** | — |
| 2:00–2:35 | **4. ハンズオン③ Grill Me系Agent Skillを自作する** — `tsumete` を育てながら、問い詰める本体`tsumetsume`と書き残す`ddd`に役割を分ける | `SKILL.md` 3ファイル |
| 2:35–2:55 | **5. ハンズオン④ SkillとHooksで開発工程を拡張する** — Skillで工程を拡張する考え方と、条件に合えば確実に処理を実行するHooksの両方に触れる | Hook 1件 |
| 2:55–3:00 | **6. 前半まとめ・宿題の説明** | — |

時間は目安です。**2〜4が前半の核心**なので、時間が押しても2・4は削らず、5を切り上げてください。**2はVibe Coding・MCP・Agent Skillの3本柱を一通り触るため65分と長めです。**
...
Claude Codeの3本柱を、順番に一通り触ります。**Vibe Coding**(頼んだら動く)→**MCP**(外部サービスとつながる)→**Agent Skill**(やり方を覚えさせる)。この3つが今日一日ずっと使う道具の基本形です。
...
| **20分** | [最初のAgent Skillを作ってみる](#最初のagent-skillを作ってみるtsumete20分)（`tsumete`） |
| **8分** | [設定を開いて中を見る](#設定を開いて中を見る)（`/permissions`・`/config`） |
| **2分** | 予備 |

### やること

新しいリポジトリ **`vibecoding-0908`** をその場で作り、Claude Codeで開いて、簡単なものを1つ、Vibe Codingで作ります。ここではあえて**仕様を厳密にしすぎません**。「こういうものが欲しい」を思いつくまま伝え、出てきたものを見て、直したいところを言葉で返す——このリズムを体で覚えるのが目的です。

1. **VS Codeで、いま開いているフォルダのままで構いません** — このあとClaude Codeには絶対パスで指示するので、今どこを開いていても問題ありません
...
> **この接続は claude.ai 側に残ります。** 外し方は [後半のクロージング](02-part2.md#今日つないだものの外し方)にあります。
...
開き直したら、送る前に一度 `/mcp` を開いて、**Gmailが接続済み（Connected）になっているか確認してください。** ここで確認せずにいきなり下書き作成を頼むと、失敗の原因が「接続できていない」のか「指示が悪い」のか切り分けにくくなります。
...
> **送信は必ず自分の意思で。** Auto Modeでもメール送信のような取り消せない操作は確認が挟まりやすいはずですが、そこは過信せず、**「下書きを作って」以上の指示は今日は出さない**でください。ファイルの操作と違って、メールは一度送ると取り消せません。

### 最初のAgent Skillを作ってみる（`tsumete`・20分）

3本柱の最後、**Agent Skill**です。「毎回同じことを言う」代わりに、**やり方そのものをファイルに保存**しておくと、以後は一言でその作法を呼び出せます。
...
`/` を打つと候補に出てきます。一問ずつ質問が返ってくれば成功です。**これがAgent Skillの正体です。** 何かを頼むたびに長い説明を書く代わりに、一度ファイルに書いておけば、以後は `/tsumete` の一言で同じ振る舞いを呼び出せます。

> **`disable-model-invocation: true` という行を入れました。** これは**「Claudeが自分の判断で勝手に起動してはいけない。人間が `/tsumete` と打ったときだけ動く」**という指定です。問い詰めは何十往復もする長い作業なので、**始めるかどうかは人間が握っておきます。** この行の意味は、ハンズオン③でもう一度扱います。

> **今日はまだ触りだけです。** この `tsumete` は、ハンズオン②・③でさらに育てていきます。完成させる必要はいまはありません。
...
**まず、自分のアプリのリポジトリを作ります。** ここが**宿題も後半も、最後までずっと使うリポジトリ**になります。前半2の `vibecoding-0908`（使い捨て）とは別の、新しいリポジトリです。

1. **VS Codeを開いたままで構いません** — `vibecoding-0908` を開いたままでも大丈夫です。Claude Codeには絶対パスで指示します
2. **`myapp` という名前で新しいリポジトリを作る** — ハンズオン①と同じ場所（`~/github/`。普段の置き場所がある人はそちらで構いません）にまとめます
   ...
   **`gh` コマンドが使える場合**
   ...
   > **なぜここだけ非公開なのか。** このあとここに書き出すのは、**あなたが何に困っているか**です。「誰が使うか」を言語化するので、職場の部署名や、まだ社外に出していないアイデアがそのまま入ります。`README.md` だけでなく、`CONTEXT.md`（用語集）や `docs/adr/`（決定記録）も**読めば事情が分かる文書**です。
   > **あとから公開に変えるのは1クリックですが、一度公開したものを無かったことにはできません。** 迷ったら非公開が正解です。
   > 前半2の `vibecoding-0908`（テトリスなど）は公開のままで構いません。**中身の性質が違います。**
...
## 4. ハンズオン③ Grill Me系Agent Skillを自作する（35分）
