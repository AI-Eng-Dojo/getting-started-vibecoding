...
2. **`myapp` という名前で新しいリポジトリを作る** — ハンズオン①と同じ場所（`~/github/`。普段の置き場所がある人はそちらで構いません）にまとめます
   ...
   **`gh`が使える場合**
   ...
   gh repo create myapp --public --clone --add-readme
   ```

   ハンズオン①と同じ形です。**後半の🚀拡張ミッションで、PRのURLを隣の人に渡してレビューしてもらう場面があります。** そこまでやりたい人は `--public` のままにしておいてください（`--private` だと相手から見えません）。

   **`gh`が使えない場合**

   ブラウザのGitHubで `myapp` という名前のリポジトリを作成し、表示されたURLをコピーして、Claude Codeに次のように貼り付けます。
